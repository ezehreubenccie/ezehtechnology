...
class CiscoFMC:
    ...
    def req(self, resource, method='get', **kwargs):
        '''
        Execute an arbitary HTTP request by supplying a resource without
    ...
    def add_object(self, obj_body):
        '''
        Creates a new generic policy object given a complete object body URL
        '''
    ...
    def add_group(self, group_dict):
        '''
        Give a complete object group, create all objects in the group and
       ...
       # All objects built; update the group's "object" key
       group_resp = self.add_object(group_dict)
       
       # Return the group response with will contain all UUIDs
       return group_resp
