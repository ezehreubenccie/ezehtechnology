...
# Retrieve existing host objects and return a dictionary {name or ip: id}
def get_existing_host_ids(token, domain_uuid):
    ...
    if response.status_code == 200:
        for host in response.json().get("items", []):
            host_ids[host["name"]] = host["id"]
            host_ids[host["value"]] = host["id"]  # Also map by IP address

    print(
        f"🔹 Retrieved {len(host_ids)} existing host objects from FMC: {list(host_ids.keys())}"
    )
    return host_ids
...
def create_host_objects(token, domain_uuid, host_ids):
    ...
    for host in HOSTS:
        if host["name"] in host_ids or host["ip"] in host_ids:
            print(f"✅ Host {host['name']} ({host['ip']}) already exists. Skipping.")
        ...
        if response.status_code in [200, 201]:
            ...
            host_ids[host["ip"]] = host_id  # Store by IP as well
    ...
    for group_name in groups:
        ...
        first_host = next(
            (
                ...
                and (host["name"] in host_ids or host["ip"] in host_ids)
            ),
            None,
        )
        if not first_host:
        ...
        first_host_id = host_ids.get(
            first_host["name"], host_ids.get(first_host["ip"], None)
        )
        if not first_host_id:
            print(
                f"❌ Could not determine a valid host ID for {first_host['name']}, skipping group creation."
            )
            continue
        ...
        data = {
            ...
            "type": "NetworkGroup",
            "objects": [{"type": "Host", "id": first_host_id}],
        }
