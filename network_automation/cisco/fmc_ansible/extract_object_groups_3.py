...
def debug_sample_network_groups(network_groups):
    """
    Debug: Print structure of a sample network group for inspection.
    """
    if network_groups:
        print("Sample network group structure:")
        print(json.dumps(network_groups[0], indent=4))
    else:
        print("No network groups found.")

def filter_network_groups_by_ips(network_groups, target_ips):
    """
    Filter network groups to find those containing the specified IPs.
    """
    matched_groups = []
    for group in network_groups:
        # Get the list of "objects" within each network group
        group_objects = group.get("objects", [])
        group_ips = {obj.get("value") for obj in group_objects if obj.get("type") == "Network"}
...
def main():
    ...
    try:
        ...
        print("Inspecting sample network group structure for debugging...")
        debug_sample_network_groups(network_groups)

        print(f"Filtering network groups for IPs: {', '.join(TARGET_IPS)}")
        matched_groups = filter_network_groups_by_ips(network_groups, TARGET_IPS)
