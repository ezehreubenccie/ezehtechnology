...
def main():
    ...
    with open("hosts/labrouters.yml", "r") as handle:
    ...
    with open("vars/routers/labrouters/labroutervars.yml", "r") as handle:
        intf = safe_load(handle)
        intf = intf["mrtlabrouter02"]
        # Setup the jinja2 templating environment and render the template
    j2_env = Environment(
        loader=FileSystemLoader("."), trim_blocks=True, autoescape=True
    )
    template = j2_env.get_template("jinja/labrouters/mrtlabrtr1_intf_conf.j2")
