...
def vlan_diff(int_vlan_list, run_vlan_dict):
    ...
    for int_vlan in int_vlan_list:
        ...
        vlan_dict = {
            "name": int_vlan["name"],
            "assg_ports": int_vlan["assg_ports"]
        ...
        if run_vlan:
            int_status = set(int_vlan["status"])
            int_assg_ports = set(int_vlan["assg_Ports"])
            int_vlan_number = set(int_vlan["vlan_number"])
            run_status = set(int_vlan["status"])
            run_assg_ports = set(int_vlan["assg_Ports"])
            run_vlan_number = set(int_vlan["vlan_number"])
            vlan_dict.update({"add_status": list(int_status - run_status)})
            vlan_dict.update({"del_status": list(run_status - int_status)})
            ...
            vlan_dict.update({"add_vlan_number": list(int_vlan_number - run_vlan_number)})
            vlan_dict.update({"del_vlan_number": list(run_vlan_number - int_vlan_number)})

        
        # intended VLAN doesn't exist, so add all the VLANs
        else:
            vlan_dict.update({"add_vlan_number": int_vlan["vlan_number"]})
            vlan_dict.update({"del_vlan_number": []})
            vlan_dict.update({"add_status": int_vlan["status"] })
            vlan_dict.update({"del_status": []})
