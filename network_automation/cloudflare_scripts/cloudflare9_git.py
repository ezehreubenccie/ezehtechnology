...
def main():
    ...
    for italy_zone, italy_zone_id in italy_dns_zone_database.items():
        
        files = {
            f'file': open(f'italy_dns_zones/{italy_zone}', 'rb'),
            'proxied': (None, 'true'),
           }

        response = requests.post(
            f'https://api.cloudflare.com/client/v4/zones/{italy_zone_id}/dns_records/import',
            headers=headers,
            files=files,
           )

        print(f'Status for {italy_zone} is {response}')
